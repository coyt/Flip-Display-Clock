...
int lasthour = 0;
int lastminute = 0;
unsigned long lastMillis = 0; //FIX TRY 1
unsigned long updateTimeMillis = 0; //FIX TRY 2
int hour = 0;
int minute = 0;
int ledMem;
...
void setup() {
    ...
    //enable watchdog timer - the Arduino will reset if watchdog is not fed atleast every 10s.
    //countdownMS shows time remaining before reset if watchdog is not fed.  
...
// Fix Try 1: just make milliseconds counter longer - done
// Fix Try 2: Only update time every 10 seconds
// Fix Try 3: add small delay in loop
//

void loop() {

    //delay loop somewhat just for stability testing
    delay(5);

    // Reset (FEED) watchdog with every loop to make sure the sketch keeps running.
    Watchdog.reset();
